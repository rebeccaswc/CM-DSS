...
import { CloudArrowUpIcon, LockClosedIcon, TrashIcon, PencilIcon, ExclamationTriangleIcon } from "@heroicons/react/24/solid";
...
function FilePage() {
  ...
  const handleEditTitle = (file) => {
    const newTitle = prompt("Enter a new title for this image:", titles[file] || "");
    if (newTitle !== null) {
      ...
      <div className="w-10/12 bg-[#1d203e] flex justify-start">
        <div className="flex flex-col items-center justify-start w-full mt-20">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 mt-8">
            {files.length > 0 ? (
              files.map((file, index) => (
                <div key={index} className="bg-white p-4 rounded-lg shadow w-64 h-64 relative overflow-hidden flex flex-col">
          ...
          <div className="bg-slate-400 text-white rounded-lg p-2 flex items-center justify-center fixed bottom-4 shadow-lg">
            ...
            <p className="text-sm">Only Cisco Packet Tracer network diagrams can be uploaded.</p>
          </div>
        </div>
      </div>
    </main>
